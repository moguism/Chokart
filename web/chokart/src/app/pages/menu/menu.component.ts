...
export class MenuComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    if (!this.authService.isAuthenticated()) {
      ...
      this.messageReceived$ = this.webSocketService.messageReceived.subscribe(
  ...
  searchFriend(queryfriend: string): void {
    ...
    this.acceptedFriends.forEach((friendship) => {
      ...
      if (friendship.senderUser) {
        encontrados = this.acceptedFriends.filter((user) =>
          user.senderUser?.nickname.includes(query)
        );
        ...
        encontrados = this.acceptedFriends.filter((user) =>
          user.receiverUser?.nickname.includes(query)
        );
        encontrados.forEach((amigo) => {
  ...
  hasFriendship(user: User): boolean {
    return this.friendsRaw.some(
      (friend) =>
        (friend.senderUserId === user.id &&
          friend.receiverUserId === this.user?.id) ||
        (friend.receiverUserId === user.id &&
          friend.senderUserId === this.user?.id)
    );
  }

  // comprueba si se le ha enviado una solicitud de amistad y esta en espera
  waitingFriendship(user: User): boolean {
    const amistad: Friend | undefined = this.friendsRaw.find(
      (friend) =>
        (friend.senderUserId === user.id &&
          friend.receiverUserId === this.user?.id) ||
        (friend.receiverUserId === user.id &&
